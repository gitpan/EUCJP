...
my $qq_paren   = qr{(?{local $nest=0}) (?>(?:
                    \\c[\x40-\x5F] | \\? \x8F[\xA1-\xFE][\xA1-\xFE]|[\8E\xA1-\xFE][\x00-\xFF] | \\ [\x00-\xFF] |
                    [^()] |
                             \(  (?{$nest++}) |
                             \)  (?(?{$nest>0})(?{$nest--})|(?!)))*) (?(?{$nest!=0})(?!))
                 }xms;
...
my $q_paren    = qr{(?{local $nest=0}) (?>(?:
                    \x8F[\xA1-\xFE][\xA1-\xFE]|[\8E\xA1-\xFE][\x00-\xFF] |
                    [^()] |
                             \(  (?{$nest++}) |
                             \)  (?(?{$nest>0})(?{$nest--})|(?!)))*) (?(?{$nest!=0})(?!))
                 }xms;
...
END
        ...
        if (s/^ \s* use \s+ EUCJP \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list !~ m/\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Eeucjp::ord'     if m/\A ord \z/oxms;
                    $function_ord_    = 'Eeucjp::ord_'    if m/\A ord \z/oxms;
                    $function_reverse = 'Eeucjp::reverse' if m/\A reverse \z/oxms;
                }
            }
        }
    }
...
# while (<WILDCARD>) --- glob
    ...
    elsif (/\G \b while \s* \( \s* < ((?:\x8F[\xA1-\xFE][\xA1-\xFE]|[\8E\xA1-\xFE][\x00-\xFF]|[^>\0\a\e\f\n\r\t])+?) > \s* \) \b /oxgc) {
...
# <WILDCARD> --- glob
    ...
    elsif (/\G < ((?:\x8F[\xA1-\xFE][\xA1-\xFE]|[\8E\xA1-\xFE][\x00-\xFF]|[^>\0\a\e\f\n\r\t])+?) > /oxgc) {
...
sub classic_character_class {
    ...
    return {
        '.'  => ($modifier =~ /s/) ?
                '(?:\x8F[\xA1-\xFE][\xA1-\xFE]|[\8E\xA1-\xFE][\x00-\xFF]|[\x00-\xFF])' :
                '(?:\x8F[\xA1-\xFE][\xA1-\xFE]|[\8E\xA1-\xFE][\x00-\xFF]|[^\n])',
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            ...
            $e_tr = qq{Eeucjp::tr($variable,$charclass,$e$charclass2,'$modifier')};
        }
    }
...
#
        ...
        # rewrite \1,\2,\3 ... --> \2,\3,\4 ...
            ...
            if (($3 eq '0') or ($2 >= 40)) {
                ...
                # join separated multiple octet
                if ($char[$i] =~ m/\A \\ (?:24[1-7]|2[567][0-7]|3[0-6][0-7]|37[0-6]) \z/oxms) {
                    if ($i < $#char) {
                        $char[$i] .= $char[$i+1];
                        splice @char, $i+1, 1;
                    }
...
sub e_s2 {
    ...
    if ($left_e > $right_e) {
        if ($modifier =~ /e/) {
            return join '',
                $delimiter,
                'Eeucjp::shift_matched_var().',     @char, '>]}' x ($left_e - $right_e),
...
the script was written in EUC-JP.
...
we're loving, so many users hope to write scripts in EUC-JP encodings; without
giving up a whole new feature of Perl5.8, Perl5.10.

The EUC-JP was developed in order to maintain backward compatibility. Generally,
